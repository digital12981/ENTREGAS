    ...
    <Dialog open={isOpen} onOpenChange={() => {}} modal={true}>
      <DialogContent className="p-0 sm:max-w-none w-full h-full max-h-screen overflow-hidden border-none shadow-none">
        ...
        <div className="relative flex flex-col justify-center items-center h-screen bg-transparent z-10">
          ...
          {/* Status Modal */}
          <div className="w-11/12 max-w-md mt-32 bg-white rounded-[3px] overflow-hidden">
            <div className="custom-section-header">
              <p className="custom-section-title text-center font-normal">{title}</p>
            </div>

            <div className="custom-input-field p-4">
              <div className="space-y-3">
                {loadingSteps.map((step, index) => (
                  <div 
                    key={index} 
                    id={`status${index+1}`} 
                    className={`status-item flex items-center p-2 bg-gray-50 rounded-[3px] ${index <= currentStep ? 'active' : ''}`}
                  >
              ...
              {isComplete && (
                <div className="mt-6 p-4 bg-green-50 rounded-[3px] border border-green-200">
