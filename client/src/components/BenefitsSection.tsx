    ...
    <section className="bg-gradient-to-b from-white to-gray-50 py-16 px-4">
      <div className="container mx-auto">
        <div className="text-center mb-10">
          <h2 className="title mb-3">Vantagens de ser um Motorista Parceiro Uber</h2>
          <div className="w-20 h-1 bg-black mx-auto rounded-full"></div>
          <p className="body-text max-w-3xl mx-auto">Junte-se a milhares de motoristas que já estão transformando seu tempo em oportunidades lucrativas</p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {benefits.map((benefit, index) => (
            <div key={index} className="bg-white p-6 rounded-lg shadow-lg transition-transform duration-300 hover:transform hover:-translate-y-2 border-t-4 border-black">
              <div className="text-black text-4xl mb-4 flex justify-center">
                <i className={benefit.icon}></i>
              </div>
              <h3 className="heading text-center">{benefit.title}</h3>
              <p className="body-text text-center">{benefit.description}</p>
            </div>
          ))}
        </div>
        
        <div className="text-center mt-12 rounded-lg p-8 text-white shadow-lg bg-black">
          <h3 className="text-xl font-bold mb-4">Você está no controle. Você define seus ganhos.</h3>
          <p className="max-w-3xl mx-auto">Na Uber, acreditamos que você merece flexibilidade e recompensas justas pelo seu trabalho. Nossa plataforma foi projetada para valorizar cada entrega que você faz, garantindo os melhores ganhos do mercado.</p>
        </div>
      </div>
    </section>
  );
